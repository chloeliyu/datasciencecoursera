...
| for storing multiple classes of data. In this case, since every
| element of the list returned by lapply() is a character vector of
| length one (i.e. "integer" and "vector"), cls_list can be
| simplified to a character vector. To do this manually, type
...
| in 'sapply') the result for you. Use sapply() the same way you used
| lapply() to get the class of each column of the flags dataset and
| store the result in cls_vect. If you need help, type ?sapply to
...
| Keep working like that and you'll get there!
  ...
  |====================                                        |  34%
| Use class(cls_vect) to confirm that sapply() simplified the result
...
| length one, then sapply() returns a vector. If the result is a list
| where every element is a vector of the same length (> 1), sapply()
| returns a matrix. If sapply() can't figure things out, then it just
...
| representing a different color. The value of the indicator variable
| is 1 if the color is present in a country's flag and 0 otherwise.

...

  |=========================                                   |  42%
| Therefore, if we want to know the total number of countries (in our
| dataset) with, for example, the color orange on their flag, we can
| just add up all of the 1s and 0s in the 'orange' column. Try
...
| is the object over which we are looping (i.e. flag_colors) and the
| second argument is the name of the function we wish to apply to
| each column (i.e. sum). Remember that the second argument is just
...
| element of this list is of length one, so the result can be
| simplified to a vector by calling sapply() instead of lapply(). Try
| it now.
...
| You got it right!
  ...
  |==================================                          |  56%
| Perhaps it's more informative to find the proportion of flags (out
| of 194) containing each color. Since each column is just a bunch of
| 1s and 0s, the arithmetic mean of each column will give us the
| proportion of 1s. (If it's not clear why, think of a simpler
| situation where you have three 1s and two 0s -- (1 + 1 + 1 + 0 +
                                                     | 0)/5 = 3/5 = 0.6).
...
| flag_colors. Remember that the second argument to sapply() should
| just specify the name of the function (i.e. mean) that you want to
| apply.
...
| simplify the result to vector. That's because each element of the
| list returned by lapply() was a vector of length one. Recall that
| sapply() instead returns a matrix when each element of the list
| returned by lapply() is a vector of the same length (> 1).

...

|=====================================                       |  62%
| To illustrate this, let's extract columns 19 through 23 from the
| flags dataset and store the result in a new data frame called
| flag_shapes. flag_shapes <- flags[, 19:23] will do it.
...
|======================================                      |  64%
| Each of these columns (i.e. variables) represents the number of
| times a particular shape or design appears on a country's flag. We
| are interested in the minimum and maximum number of times each
...
| argument, which should be a numeric vector. Use lapply() to apply
| the range function to each column of flag_shapes. Don't worry about
...
| That's the answer I was looking for.
  ...
  |===========================================                 |  72%
| Each column of shape_mat gives the minimum (row 1) and maximum (row
...
| the examples we've looked at so far. Let's look at an example where
| sapply() can't figure out how to simplify the result and thus
...
| function to each column in the flags dataset, storing the result in
...
$mainhue
[1] "green"  "red"    "blue"   "gold"   "white"  "orange" "black" 
[8] "brown" 
...
| number of unique values for each variable). Simplify the result, if
| possible. Hint: Apply the length() function to each element of
| unique_vals.
...
| You nailed it! Good job!
  ...
  |====================================================        |  86%
| The fact that the elements of the unique_vals list are all vectors
| of *different* length poses a problem for sapply(), since there's
| no obvious way of simplifying the result.
...
| defined, thus requiring you to write your own. Writing functions in
| R is beyond the scope of this lesson, but let's look at a quick
| example of how you might do so in the context of loop functions.
...
| element of the unique_vals list is a vector and we're not aware of
| any built-in function in R that returns the second element of a
...
| just a 'dummy variable' that takes on the value of each element of
| unique_vals, in turn.
...
| All that hard work is paying off!
  ...
  |==========================================================  |  96%
| The only difference between previous examples and this one is that
| we are defining and using our own function right in the call to
| lapply(). Our function has no name and disappears as soon as
...
| list. In the next lesson, we'll take a look at some close relatives
...
What is your assignment token? nAGbb9iHvognwt2E
...
| In the last lesson, you learned about the two most fundamental
| members of R's *apply family of functions: lapply() and sapply().
| Both take a list as input, apply a function to each element of the
| list, then combine and return the result. lapply() always returns a
| list, whereas sapply() attempts to simplify the result.

...

|==                                                          |   4%
| In this lesson, you'll learn how to use vapply() and tapply(), each
| of which serves a very specific purpose within the
| Split-Apply-Combine methodology. For consistency, we'll use the
...
|=======                                                     |  12%
| I've stored the data in a variable called flags. If it's been a
| while since you completed the 'lapply and sapply' lesson, you may
| want to reacquaint yourself with the data by using functions like
| dim(), head(), str(), and summary() when you return to the prompt
| (>). You can also type viewinfo() at the prompt to bring up some
| documentation for the dataset. Let's get started!

...

  |==========                                                  |  16%
| As you saw in the last lesson, the unique() function returns a
| vector of the unique values contained in the object passed to it.
| Therefore, sapply(flags, unique) returns a list containing one
| vector of unique values for each column of the flags dataset. Try
| it again now.
...
| Keep working like that and you'll get there!
  ...
  |============                                                |  20%
| What if you had forgotten how unique() works and mistakenly thought
| it returns the *number* of unique values contained in the object
| passed to it? Then you might have incorrectly expected
| sapply(flags, unique) to return a numeric vector, since each
| element of the list returned would contain a single number and
...
| recognize your mistake. However, when working non-interactively
| (e.g. writing your own functions), a misunderstanding may go
| undetected and cause incorrect results later on. Therefore, you may
| wish to be more careful and that's where vapply() is useful.
...
| the operation to stop. This can prevent significant problems in
| your code that might be caused by getting unexpected return values
| from sapply().

...

|===================                                         |  32%
| Try vapply(flags, unique, numeric(1)), which says that you expect
| each element of the result to be a numeric vector of length 1.
| Since this is NOT actually the case, YOU WILL GET AN ERROR. Once
| you get the error, type ok() to continue to the next question.
...
| Keep up the great work!
  ...
  |======================                                      |  36%
| Recall from the previous lesson that sapply(flags, class) will
| return a character vector containing the class of each column in
| the dataset. Try that again now to see the result.
...
| the vapply() result is identical to the sapply() result -- a
| character vector of column classes.

...

|=============================                               |  48%
| You might think of vapply() as being 'safer' than sapply(), since
| it requires you to specify the format of the output in advance,
| instead of just allowing R to 'guess' what you wanted. In addition,
| vapply() may perform faster than sapply() for large datasets.
| However, when doing data analysis interactively (at the prompt),
...
| groups based on the value of some variable, then apply a function
| to the members of each group. The next function we'll look at,
...
| world. Use table(flags$landmass) to see how many flags/countries
...
| That's the answer I was looking for.
  ...
  |======================================                      |  64%
| The 'animate' variable in our dataset takes the value 1 if a
| country's flag contains an animate image (e.g. an eagle, a tree, a
                                            | human hand) and 0 otherwise. Use table(flags$animate) to see how
...
| separately for each of the six landmass groups, thus giving us the
| proportion of flags containing an animate image WITHIN each
| landmass group.
...
| Similarly, we can look at a summary of population values (in round
                                                            | millions) for countries with and without the color red on their
| flag with tapply(flags$population, flags$red, summary).
...
| Excellent work!
  ...
  |=======================================================     |  92%
| What is the maximum population (in millions) for the fourth
| landmass group (Africa)?
...
| All that practice is paying off!
  ...
  |==========================================================  |  96%
| In this lesson, you learned how to use vapply() as a safer
| alternative to sapply(), which is most helpful when writing your
| own functions. You also learned how to use tapply() to split your
| data into groups based on the value of some variable, then apply a
| function to each group. These functions will come in handy on your
...
Selection: 1
What is your email address? yli57@buffalo.edu
What is your assignment token? FWdjFZ9920AfMzAt
Grade submission succeeded!
